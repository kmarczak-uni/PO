...
### Zadanie
1. Napisz klasę reprezentującą koło, z polami dlugość promienia i współrzędnymi środka `x` oraz `y`. Klasa ma posiadać jeden konstruktor domyślny z listą inicjalizacyjną (np. środek w punkcie `[0, 0]` i długość promienia `1`) oraz wieloargumentowy. Stwórz po jednym obiekcie z użyciem każdego konstruktora.  
...
# Zadanie domowe
1. Zmodyfikuj program z poprzedniego zadania domowego tak, aby to użytkownik sam mógł wprowadzić z klawiatury liczbę zadań, które chce zapisać na swojej liście.
2. Napisz klasę Triangle, która zamodeluje obiekty trójkątów. Klasa ma zawierać tylko wymiary trójkąta potrzebne do policzenia jego pola. Zwróc uwagę na prawidłowe nazewnictwo pól.
